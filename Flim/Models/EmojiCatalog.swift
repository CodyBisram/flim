...
/// The CLDR annotation keywords, bundled as a resource and keyed by VS16-stripped skeleton.
///
/// This is the same Unicode data Apple derives Messages' emoji search from, which is the bar
/// this picker is held to: "laugh" finding everything that laughs, "bday" finding the cake. The
/// formal Unicode name (already a token source below) says what a glyph IS; these say what
/// people MEAN when they reach for it (lmao, rofl, yolo). Generated by
/// scripts/gen_emoji_keywords.py; re-run it to pick up a new CLDR release.
///
/// A bundled file rather than a Swift literal on purpose: 38k keywords as a dictionary literal
/// is the kind of expression that already defeated the type checker once in this codebase, and
/// it would recompile on every touch of this file.
private func cldrKeywordsBySkeleton() -> [String: [String]] {
    guard let url = Bundle.main.url(forResource: "emoji-keywords", withExtension: "txt"),
          let raw = try? String(contentsOf: url, encoding: .utf8) else { return [:] }
    var out: [String: [String]] = [:]
    out.reserveCapacity(4500)
    for line in raw.split(separator: "\n") {
        guard let tab = line.firstIndex(of: "\t") else { continue }
        let keywords = line[line.index(after: tab)...].split(separator: "|").map(String.init)
        out[String(line[..<tab])] = keywords
    }
    return out
}

/// The emoji with every variation selector removed: the form both sides of the CLDR lookup are
/// normalised to, since the catalog generates some emoji WITH a VS16 the annotations key without.
private func emojiSkeleton(_ text: String) -> String {
    String(String.UnicodeScalarView(text.unicodeScalars.filter { $0.value != 0xFE0F }))
}
...
private func generate() -> EmojiCatalogData {
    ...
    func add(_ text: String, to category: String, tokens: [String]) {
        ...
        // Name tokens first (what it is), then CLDR (what people mean), then the hand aliases
        // (vernacular CLDR lacks). Order matters only for readability in debugging; matching
        // treats the list as a set.
        let all = tokens + (cldr[emojiSkeleton(text)] ?? []) + (aliases[text] ?? [])
        for token in withStems(all) where dedupedSeen.insert(token).inserted {
